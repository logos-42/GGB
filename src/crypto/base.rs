...
mod tests {
    ...
    fn test_crypto_suite_basic() {
        ...
        // 获取地址
        let eth_addr = suite.eth_address();
        let sol_addr = suite.sol_address();
        
        assert!(eth_addr.starts_with("0x"));
        assert!(eth_addr.len() == 42); // 0x + 40 hex chars
    ...
    #[test]
    fn test_crypto_suite_with_seed() {
        let eth_seed = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
        let sol_seed = "5uHqkH1kq3qjJcK7vqkH1kq3qjJcK7vqkH1kq3qjJcK7vqkH1kq3qjJcK7v"; // base58 encoded 32 bytes
        
        let config = CryptoConfig {
            eth_hex_seed: Some(eth_seed.to_string()),
        ...
        // 使用相同种子应该生成相同的地址
        let suite2 = CryptoSuite::new(CryptoConfig {
            eth_hex_seed: Some(eth_seed.to_string()),
            sol_bs58_seed: Some(sol_seed.to_string()),
        }).unwrap();
        
        assert_eq!(suite.eth_address(), suite2.eth_address());
    ...
    fn test_keccak_hash() {
        ...
        // 相同输入应该产生相同输出
        let hash2 = keccak(data);
        assert_eq!(hash, hash2);
        
        // 不同输入应该产生不同输出
        let hash3 = keccak(b"hello world!");
