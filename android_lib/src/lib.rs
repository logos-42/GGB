...
use jni::sys::{jlong, jint, jboolean, jfloat, jstring};
...
// JNI辅助函数
fn jstring_to_rust_string(env: &mut JNIEnv, j_string: JString) -> Result<String, jni::errors::Error> {
    let java_string: String = env.get_string(&j_string)?.into();
    Ok(java_string)
}
...
pub unsafe extern "C" fn williw_node_set_device_callback(
    ...
    let handle = &mut *ptr;
    handle.device_callback = callback;
...
pub unsafe extern "system" fn Java_com_williw_mobile_WilliwNode_nativeGetCapabilities(
    ...
    let result = match env.new_string(json_str) {
        Ok(jstring) => jstring.into_raw() as jstring,
        Err(_) => ptr::null_mut(),
    };
...
pub unsafe extern "system" fn Java_com_williw_mobile_WilliwNode_nativeUpdateNetworkType(
    ...
    let net_str = match jstring_to_rust_string(&mut env, network_type) {
        Ok(s) => s,
